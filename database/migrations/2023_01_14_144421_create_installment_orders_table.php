...
class CreateInstallmentOrdersTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::table('accounting', function (Blueprint $table) {
            // Check if 'product_id' column exists before adding 'installment_payment_id'
            if (!Schema::hasColumn('accounting', 'product_id')) {
                $table->integer('product_id')->unsigned()->nullable();
            }
            if (!Schema::hasColumn('accounting', 'type')) {
                $table->string('type')->nullable(); // Corrected from varchar to string
            }
            ...
            $table->foreign('installment_payment_id')->references('id')->on('installment_order_payments')->onDelete('cascade'); // Corrected foreign key syntax
        });
        
        Schema::table('sales', function (Blueprint $table) {
            // Check if 'registration_package_id' column exists before adding 'installment_payment_id'
            if (!Schema::hasColumn('sales', 'registration_package_id')) {
                $table->integer('registration_package_id')->unsigned()->nullable();
            }
            if (!Schema::hasColumn('sales', 'payment_method')) {
                $table->string('payment_method')->nullable(); // Corrected from varchar to string
            }
